...
def main(task_type: str = 'binary',
    ...
    # 设置日志记录器
    logger = get_training_logger("transformer", task_type)
    logger.info(f"开始 Transformer 模型{'二分类' if task_type == 'binary' else '多分类'}任务")
    logger.info(f"模型配置: num_heads={num_heads}, num_layers={num_layers}")
    ...
    try:
        ...
        # 创建 Transformer 模型
        logger.info("正在创建 Transformer 模型...")
        ...
        # 使用通用训练器框架
        logger.info("正在创建训练器...")
        trainer = TrainerBuilder(model, model_name='transformer', task_type=task_type) \
        ...
        # 保存模型
        model_path = f"./models/transformer_{task_type}_model.pth"
