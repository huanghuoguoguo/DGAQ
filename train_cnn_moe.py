...
"""
CNN-MoE模型训练脚本 - 极简正确版
与 LightweightCNN 完全兼容，仅需将模型类替换即可
"""
...
def main(task_type: str = 'binary',
         ...
         num_experts: int = 3,
         aux_weight: float = 0.3,          # 调大辅助损失，让门控更快学习
         balance_weight: float = 1e-2):
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logger = get_training_logger("cnn_moe", task_type)
    logger.info(f"开始CNN-MoE模型{'二分类' if task_type == 'binary' else '多分类'}任务")
    logger.info(f"专家数量: {num_experts}, 辅助损失权重: {aux_weight}, 负载均衡权重: {balance_weight}")
    ...
    trainer = TrainerBuilder(model, model_name='cnn_moe', task_type=task_type) \
    ...
    def train_one_epoch_moe(loader, is_train=True):
        ...
        for batch_idx, (data, targets) in enumerate(loader):
            ...
            if is_train:
                ...
                logits, gate_info = model(data, return_gate=True)
    ...
    for epoch in range(epochs):
        ...
        logger.info(f'Train: loss={train_loss:.4f} acc={train_acc:.4f} usage={train_usage.tolist()}')
        logger.info(f'Val:   loss={val_loss:.4f} acc={val_acc:.4f} usage={val_usage.tolist()}')
        
        # 早停
        if val_acc > best_acc:
            best_acc, patience_counter = val_acc, 0
            model_path = f'./models/cnn_moe_best_{task_type}.pth'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CNN-MoE模型训练 - 极简正确版')
