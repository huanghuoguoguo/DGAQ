...
# -*- coding: utf-8 -*-
"""
Mamba2-MoE 模型训练脚本 - 极简正确版
Mamba2 (SSD算法) + MoE 稀疏激活
"""
...
def main(task_type: str = 'binary',
    ...
    if not torch.cuda.is_available():
        print("❌ 错误: Mamba2-MoE 模型需要 CUDA 环境")
        return
    
    device = torch.device('cuda')
    logger = get_training_logger("mamba2_moe", task_type)
    logger.info(f"开始 Mamba2-MoE 模型{'二分类' if task_type == 'binary' else '多分类'}任务")
    logger.info(f"模型配置: num_layers={num_layers}, num_experts={num_experts}, d_state={d_state}, headdim={headdim}")
    ...
    model = LightweightMamba2MoE(
        ...
        embedding_dim=256,  # Mamba2需要较大嵌入维度以满足对齐要求
    ...
    # 3. 创建训练器（criterion仅作占位）
    trainer = TrainerBuilder(model, model_name='mamba2_moe', task_type=task_type) \
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            ...
            model_path = f'./models/mamba2_moe_best_{task_type}.pth'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Mamba2-MoE 模型训练')
