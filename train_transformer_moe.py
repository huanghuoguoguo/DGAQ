...
# -*- coding: utf-8 -*-
"""
Transformer-MoE 模型训练脚本 - 极简正确版
使用 Token 级稀疏 MoE 替换 FFN
"""
...
def main(task_type: str = 'binary',
    ...
    logger = get_training_logger("transformer_moe", task_type)
    logger.info(f"开始 Transformer-MoE 模型{'二分类' if task_type == 'binary' else '多分类'}任务")
    logger.info(f"模型配置: num_heads={num_heads}, num_layers={num_layers}, num_experts={num_experts}")
    logger.info(f"损失权重: aux_weight={aux_weight}, balance_weight={balance_weight}")
    ...
    # 2. 创建模型（权重在这里唯一定义）
    model = LightweightTransformerMoE(
    ...
    trainer = TrainerBuilder(model, model_name='transformer_moe', task_type=task_type) \
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            ...
            model_path = f'./models/transformer_moe_best_{task_type}.pth'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Transformer-MoE 模型训练')
    ...
    parser.add_argument('--aux_weight', type=float, default=1e-2)
